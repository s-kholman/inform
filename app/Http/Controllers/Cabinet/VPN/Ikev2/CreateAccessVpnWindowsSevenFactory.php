...
class CreateAccessVpnWindowsSevenFactory implements IkeVpnMikrotikInterface
{
    ...
    public function action(MikrotikController $mikrotikController, IkeReport $report)
    {
        ...
        $mikrotikController->modeConfigGet()->checkIpModeConfig()->modeConfigAddStaticIP();
        ...
        $script = new ScriptWindowsSevenFactory();
    ...
    {
        $archive = new Archive();
        $archive(['script_'.$mikrotikController->sslActive[0]['name'].'.cmd', 'ssl_'.$mikrotikController->sslActive[0]['name'].'.p12'],

    ...
    <div class="container">
        ...
        <table class="table table-bordered text-center caption-top">
            ...
            <tr>
                ...
                @endif
                @if($monitoring->groupBy('temperature_point_four')->count() > 1)
                        <th class="vertical-align" ><label >Точка замера №4</label></th>
                @endif
                @if($monitoring->groupBy('temperature_point_five')->count() > 1)
                        <th class="vertical-align" ><label >Точка замера №5</label></th>
                @endif
                @if($monitoring->groupBy('temperature_point_six')->count() > 1)
                        <th class="vertical-align" ><label >Точка замера №6</label></th>
                @endif
                @if($monitoring->groupBy('temperature_point_seven')->count() > 1)
                        <th class="vertical-align" ><label >Точка замера №7</label></th>
                @endif
                @if($monitoring->groupBy('temperature_point_eight')->count() > 1)
                        <th class="vertical-align" ><label >Точка замера №8</label></th>
                @endif
                @if($monitoring->groupBy('temperature_point_nine')->count() > 1)
                        <th class="vertical-align" ><label >Точка замера №9</label></th>
                @endif
                @if($monitoring->groupBy('temperature_point_ten')->count() > 1)
                        <th class="vertical-align" ><label >Точка замера №10</label></th>
                @endif
                @if($monitoring->groupBy('temperature_point_eleven')->count() > 1)
                        <th class="vertical-align" ><label >Точка замера №11</label></th>
                @endif
                @if($monitoring->groupBy('temperature_point_twelve')->count() > 1)
